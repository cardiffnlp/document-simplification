...
class AggMeticEditNoComplex:

    def __init__(self, sent_metric):
        self.name = "Aggregation Metric Edit No Complex -" + sent_metric.name
    ...
    def compute_metric_single(self, complex, simplified, references):
        ...
        for reference in references:
            deduped_sent_groups, textC_sents, textS_sents = edit_alignment(self.bic, simplified, reference)
            all_cands, all_refs = get_sents(deduped_sent_groups, textC_sents, textS_sents)           
            all_comps = ["" for _ in all_cands]    
